/*
 * irrigation_system.cpp
 *
 *  Created on: 07.10.2020
 *      Author: Mati
 */

//TODO: Irrigation system builder based on builder design pattern
...
float IrrigationSector::getPlantHealth(const std::string & _name){
	...
	if(this->plantsCount > 0)
	{
		...
		{
			if (_name.compare(this->vPlants.at(i).getName())){
				return this->vPlants.at(i).getMoisturePercent();
			}
		}
	}
...
float IrrigationSector::getPlantHealth(const uint8_t & _id){
	...
	if(this->plantsCount > 0)
	{
		for (uint8_t i = 0; i < this->plantsCount; ++i)
		{
			if (this->vPlants.at(i).getId() == _id){
				return this->vPlants.at(i).getMoisturePercent();
			}
		}
	}
...
std::string IrrigationSector::getPlantNameByID(const uint8_t & _id){
	...
	if(this->plantsCount > 0)
	{
		...
		{
			if (this->vPlants.at(i).getId() == _id){
				return this->vPlants.at(i).getName();
			}
		}
	}
